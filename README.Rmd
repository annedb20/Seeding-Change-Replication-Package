...
Data and selected code used to assess USDA financial assistance program funding in Seeding Change to Manage Climate: Growing Insights from Four USDA Programs to Support Climate-Smart Agriculture by Elinor Benami, Anne Bell, Kent D. Messer, Wei Zhang, and Michael Cecil. (link TBD)

### Data Download

The full data download can be found here (link to Harvard Dataverse to be added).
...
PCSC_data_with_HUP_info retains the information from Partnership_for_Climate_Smart_Commodities_Projects.xlsx with a binary column representing whether the project explicitly states that 50% or more of producers will be HUP or small, AND/OR that 50% or more of its funds will support HUP or small producers. Column added by Anne Bell.

Program Funding and Outlays.tab is a manual creation derived from CBO data and projections, compiled by Anne Bell.
...
* scripts/1_load_data(link TBD) - Script for loading data from dataverse.

* scripts/2_data_cleaning (link TBD) - Script for cleaning loaded data and preparing it for analysis.

* scripts/3_figures (link TBD) - Scripts for generating figures.

* scripts/4_tables (link TBD) - Scripts for generating select tables.

* scripts/5-stats (link TBD) - Scripts for generating select statistics.
